...
python -m sdoc.run_all --engine rules  # 520 emails -> out/submission.json (+ out/results.json); drop --engine to use a live key across all 520 (mind its daily RPD budget - see below)
...
* Key lookup: `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) from the environment or `backend/.env`. Force either engine explicitly with `SDOC_ENGINE=rules|gemini` or `?force_engine=` on the API.
* Every trace event and every Result carries the engine that really produced it. Asking for `gemini` without a key transparently *reports* `rules`; nothing is ever labelled `gemini` that was not.
...
* **Throttling, two layers (reviews/judge.md #6):** (1) *pre-emptive* - `ratelimit.py`'s `RateLimiter` makes every call wait until it's safely under the model's real RPM/TPM/RPD ceiling before it's ever sent (numbers per model in `_MODEL_LIMITS`, sourced from this project's own account dashboard - a free tier, single digits to low teens RPM and 20-500 RPD depending on model; override with `SDOC_GEMINI_RPM`/`_TPM`/`_RPD`). (2) *reactive* - a single `GeminiClient` shared per pipeline run (classifier + all 7 field agents) also caps in-flight calls with `asyncio.Semaphore(SDOC_GEMINI_CONCURRENCY)` (default 3), and any attempt that still fails backs off exponentially with jitter (`0.8 * 2^attempt` normally, `4.0 * 2^attempt` specifically on a 429/`RESOURCE_EXHAUSTED`, honouring a server-suggested retry delay when present).
* **Gemini path**: verified against the live API on `gemini-3.5-flash-lite` (the best RPM/RPD combination available on this tier) - classifier and all 7 field agents ran real successful calls across a 14-email mix (clean matches, real mismatches, both deterministic-trigger and fly-gate-decided `NEEDS_REVIEW`, non-comparison categories), zero errors, results matching the independently-derived rules-engine values. Built against `google-genai` 2.18 (`client.aio.models.generate_content`, `response_mime_type=application/json`, `response_json_schema`); evidence strings returned by the model are verified to occur in the document, unverifiable evidence caps confidence (the gate sees it), and compare stays a pure function - the LLM never decides match/mismatch. Mind the RPD budget above before pointing `process_all` (520 emails x up to 8 calls) at a live key - a single email or a short demo run is what this tier is sized for.
