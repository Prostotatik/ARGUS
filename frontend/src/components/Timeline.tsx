...
export default function Timeline({ nodes, active, playing, result, title, totalMs }: Props) {
  ...
  const steps = [
    ...
    { id: 'aggregated', label: 'Aggregate + compare', Icon: Network, state: st(nodes.aggregator, active), t: tPlus(nodes.compare?.state === 'done' ? nodes.compare : nodes.aggregator), sub: nodes.compare?.state === 'skipped' || nodes.aggregator?.state === 'skipped' ? 'not run' : cmp && cmp.length ? (bad ? `${bad} mismatch${bad > 1 ? 'es' : ''}` : 'all 7 match') : '', eng: nodes.aggregator?.engine ?? null, dur: nodes.aggregator?.duration_ms },
